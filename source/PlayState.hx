...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'unoriginal':
				dialogue = CoolUtil.coolTextFile(Paths.txt('unoriginal/minus8'));
			case 'banana-dance':
				dialogue = CoolUtil.coolTextFile(Paths.txt('banana-dance/futa'));
			case 'tutu':
				dialogue = CoolUtil.coolTextFile(Paths.txt('tutu/shibari'));
			case 'cbt':
				dialogue = CoolUtil.coolTextFile(Paths.txt('cbt/zone-tan'));
			case 'cabbage-jack':
				dialogue = CoolUtil.coolTextFile(Paths.txt('cabbage-jack/ecchi'));
			case 'nothin-much':
				dialogue = CoolUtil.coolTextFile(Paths.txt('nothin-much/offbrand-kawaisprite'));
		}
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'tutu' | 'banana-dance' | 'unoriginal':
			{
					defaultCamZoom = 0.9;
					curStage = 'sand wilderness';
					var bg:FlxSprite = new FlxSprite(-600, -250).loadGraphic(Paths.image('desertpractice'));
					bg.antialiasing = true;
